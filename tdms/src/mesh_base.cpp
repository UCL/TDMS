...
void triangulatePlane(int I0, int I1, int J0, int J1, int K, int coordmap[], int order,
  ...
  */
  ...
  if (!(temp_res[0] && temp_res[1] && temp_res[2]) || (coordmap[0] == coordmap[1]) ||
    ...
    snprintf(buffer, 100, "Error in triangulatePlane(), coordmap incorrect [%d %d %d], [%d %d %d]",
             coordmap[0], coordmap[1], coordmap[2], temp_res[0], temp_res[1], temp_res[2]);
    mexErrMsgTxt(buffer);
  }
...
void conciseTriangulateCuboid(int I0, int I1, int J0, int J1, int K0, int K1, mxArray **vertices,
  ...
  mxArray *triangles[6];
  mxArray *index_map;
  int ndims, dims[3], ***index_map_int, nindices, **vertices_int, i, **facets_int, j, k,
  ...
  */
  ...
  nindices = (I1 - I0 + 1) * (J1 - J0 + 1) * 2 + (I1 - I0 + 1) * (K1 - K0 - 1) * 2 +
             (J1 - J0 - 1) * (K1 - K0 - 1) * 2;
...
void conciseCreateBoundary(int I0, int I1, int K0, int K1, mxArray **vertices, mxArray **facets) {
  ...
  for (int i = 0; i < (Nverts + 4); i++) verts_t[i] = (int *) malloc(3 * sizeof(int));

  facets_t = (int **) malloc((Nfacets) * sizeof(int *));
  for (int i = 0; i < (Nfacets); i++) facets_t[i] = (int *) malloc(3 * sizeof(int));
  ...
  */
  ...
  for (int i = 0; i < (I1 - I0 + 1); i++) ind_map[i] = (int *) malloc((K1 - K0 + 1) * sizeof(int));
...
void conciseTriangulateCuboidSkip(int I0, int I1, int J0, int J1, int K0, int K1,
  ...
  mxArray *triangles[6];
  mxArray *index_map;
  int ndims, ***index_map_int, nindices, **vertices_int, i, **facets_int, j, k, **triangles_int, ii,
          jj, kk;
  int ndims_v;//, dims_v[2];
  int ndims_f;//, dims_f[2];
  //  int *dims_t;
  ...
  *vertices = mxCreateNumericArray(ndims_v, (const mwSize *) dims_v, mxINT32_CLASS, mxREAL);
  ...
  if ((I1 - I0) < dI || (J1 - J0) < dJ || (K1 - K0) < dK) dims_f[0] = dims_f[0] / 2;
  dims_f[1] = 3;
  *facets = mxCreateNumericArray(ndims_f, (const mwSize *) dims_f, mxINT32_CLASS, mxREAL);
  facets_int = cast_matlab_2D_array((int *) mxGetPr(*facets), dims_f[0], dims_f[1]);
