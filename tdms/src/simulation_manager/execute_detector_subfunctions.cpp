...
void SimulationManager::compute_detector_functions(int tind,
  ...
  // mode, with exdetintegral flagged
  ...
  if ((tind - inputs.params.start_tind) % inputs.params.Np == 0) {
    ...
    /* First need to sum up the Ex and Ey values on a plane ready for FFT.
     ...
     column major format */
    for (int j = inputs.params.pml.Dyl; j < (J_tot - inputs.params.pml.Dyu);
         j++)
    ...
    for (int im = 0; im < inputs.D_tilde.num_det_modes(); im++) {
      ...
      for (int j = 0;
        ...
        for (int i = 0;
          ...
          int m = j +
                  i * (J_tot - inputs.params.pml.Dyu - inputs.params.pml.Dyl);
                ...
                if (!inputs.params.air_interface_present) {
                  ...
                  kprop = exp(IMAGINARY_UNIT *
                          ...
                          exp(IMAGINARY_UNIT * inputs.params.air_interface *
                              2. * DCPI / lambda_an_t *
                              sqrt(1. -
                                   pow(lambda_an_t * inputs.f_vec.x[i], 2.) -
