...
interp_scheme determineInterpScheme(int cells_in_direction, int cell_id) {
  ...
  // Yee cell with index <=0 doesn't exist (indexing starts from 1)
    ...
    throw out_of_range("Error: Interior Yee cell index <=0 requested (must be >=1).\n");
  }
...
/**
 * @brief Performs band-limited interpolation on the equidistant sample values a0,...,a7, returning the interpolated value located between a3 and a4 (a3.5)
 * 
 * @param a0,a1,a2,a3,a4,a5,a6,a7 Evenly-spaced function samples 
 * @return double Interpolated function value at midpoint of a3 and a4
 */
double BLi(double a0, double a1, double a2, double a3, double a4, double a5, double a6, double a7) {
  /*Array for performing bandwidth limited interpolation obtained using Matlab's interp function */
  const int Nbvec = 8;
  const double bvec[Nbvec] = {-0.006777513830606, 0.039457774230186, -0.142658093428622, 0.609836360661632, 0.609836360661632, -0.142658093428622, 0.039457774230186, -0.006777513830606};
  ...
  // perform interpolation loop
  interp_val += a0 * bvec[0];
  interp_val += a1 * bvec[1];
  interp_val += a2 * bvec[2];
  interp_val += a3 * bvec[3];
  interp_val += a4 * bvec[4];
  interp_val += a5 * bvec[5];
  interp_val += a6 * bvec[6];
  interp_val += a7 * bvec[7];
  // return
  return interp_val;
}

/**
 * @brief Performs band-limited interpolation on the equidistant sample values stored in the array a, returning the interpolated value located between the 3rd and 4th indexed elements
 *
 * @param a Array of evenly-spaced function samples, must be of length at least 8
 * @return double Interpolated function value at midpoint of 3rd and 4th indexed elements
 */
double BLi(double *a) {
  /*Array for performing bandwidth limited interpolation obtained using Matlab's interp function */
  const int Nbvec = 8;
...
void interpolateTimeDomainHx(double ***Hxy, double ***Hxz, int i, int j, int k, int I, int J, int K, double *Hx) {
  // FOR THE TIME BEING, ASSUME 3D SIMULATION. IF 2D, REMOVE THE OPTION TO INTERPOLATE IN THE Y-DIRECTION!

  // can we interpolate in the x-direction using BLi? We need i+3 and i-4 to be valid indicies for this
  int xDirScheme = determineInterpScheme(I, i);
  // xdirScheme = BAND_LIMITED implies that we can apply band-limiting in this dimension
  int yDirScheme = determineInterpScheme(J, j);
  ...
  if (yDirScheme < zDirScheme) {
    ...
    switch (yDirScheme)
    {
        ...
        case BAND_LIMITED:
          ...
          for(int jj=up; jj<=low; jj++) {
            Hx_interps_bli[jj - low] = BLi(Hxy[k][jj][i - 4] + Hxz[k][jj][i - 4],
                                         ...
                                         Hxy[k][jj][i + 1] + Hxz[k][jj][i + 1],
                                         Hxy[k][jj][i + 2] + Hxz[k][jj][i + 2],
                                         Hxy[k][jj][i + 3] + Hxz[k][jj][i + 3]); // BLi to get Hx[i-1/2][jj][k];
          }
          break;
        case INTERP1:
          ...
          break;
        case INTERP2:
          // we are doing left-cubic interpolation for x, but BLi in the y-direction
          ...
          break;
        case INTERP3:
          // we are doing right-cubic interpolation for x, but BLi in the y-direction
      ...
      case INTERP1:
        ...
        {
        case BAND_LIMITED:
          // we are doing BLI in the x-direction, and interp1 in the y-direction
          for (int jj = low; jj <= low; jj++)
          {
          ...
          break;
        case INTERP1:
          // we are doing interp1 in both the x and y directions
          for(int jj=low; jj<=low;jj++) {
          ...
          break;
        case INTERP2:
          // we are doing interp2 in x and interp1 in y
          for (int jj = low; jj <= low; jj++)
          {
          ...
          break;
        case INTERP3:
          // we are doing interp3 in x and interp1 in y
          for (int jj = low; jj <= low; jj++)
          {
        ...
        // now use interp1 to extract Hx at the origin of this Yee cell
        *Hx = interp1(Hx_interps[0], Hx_interps[1], Hx_interps[2], Hx_interps[3]);
      case INTERP2:
        ...
        {
        case BAND_LIMITED:
          // we are doing BLI in the x-direction, and interp2 in the y-direction
          for (int jj = low; jj <= low; jj++)
          {
            Hx_interps[jj - low] = BLi(Hxy[k][jj][i - 4] + Hxz[k][jj][i - 4],
                                         Hxy[k][jj][i - 3] + Hxz[k][jj][i - 3],
          ...
          break;
        case INTERP1:
          // we are doing interp1 in x and interp2 in y
          for (int jj = low; jj <= low; jj++)
          {
            Hx_interps[jj - low] = interp1(Hxy[k][jj][i - 2] + Hxz[k][jj][i - 2],
                                             Hxy[k][jj][i - 1] + Hxz[k][jj][i - 1],
          ...
          break;
        case INTERP2:
          // we are doing interp2 in both directions
          for (int jj = low; jj <= low; jj++)
          {
            Hx_interps[jj - low] = interp2(Hxy[k][jj][i - 1] + Hxz[k][jj][i - 1],
                                             Hxy[k][jj][i] + Hxz[k][jj][i],
          ...
          break;
        case INTERP3:
          // we are doing interp3 in x and interp2 in y
          for (int jj = low; jj <= low; jj++)
          {
            Hx_interps[jj - low] = interp3(Hxy[k][jj][i - 3] + Hxz[k][jj][i - 3],
                                             Hxy[k][jj][i - 2] + Hxz[k][jj][i - 2],
        ...
        // now use interp2 to extract Hx at the origin of this Yee cell
        *Hx = interp2(Hx_interps[0], Hx_interps[1], Hx_interps[2], Hx_interps[3]);
      case INTERP3:
        ...
        {
        case BAND_LIMITED:
          // we are doing BLI in the x-direction, and interp3 in the y-direction
          for (int jj = low; jj <= low; jj++)
          {
            Hx_interps[jj - low] = BLi(Hxy[k][jj][i - 4] + Hxz[k][jj][i - 4],
                                         Hxy[k][jj][i - 3] + Hxz[k][jj][i - 3],
          ...
          break;
        case INTERP1:
          // we are doing interp1 in x and interp3 in y
          for (int jj = low; jj <= low; jj++)
          {
            Hx_interps[jj - low] = interp1(Hxy[k][jj][i - 2] + Hxz[k][jj][i - 2],
                                             Hxy[k][jj][i - 1] + Hxz[k][jj][i - 1],
          ...
          break;
        case INTERP2:
          // we are doing interp2 in x and interp3 in y
          for (int jj = low; jj <= low; jj++)
          {
            Hx_interps[jj - low] = interp2(Hxy[k][jj][i - 1] + Hxz[k][jj][i - 1],
                                             Hxy[k][jj][i] + Hxz[k][jj][i],
          ...
          break;
        case INTERP3:
          // we are doing interp3 in both directions
          for (int jj = low; jj <= low; jj++)
          {
            Hx_interps[jj - low] = interp3(Hxy[k][jj][i - 3] + Hxz[k][jj][i - 3],
                                             Hxy[k][jj][i - 2] + Hxz[k][jj][i - 2],
        ...
        // now use interp3 to extract Hx at the origin of this Yee cell
        *Hx = interp3(Hx_interps[0], Hx_interps[1], Hx_interps[2], Hx_interps[3]);
      }
  }
  else {
    // z's scheme is better (or not worse) than y's scheme
    switch (zDirScheme)
    {
    case BAND_LIMITED:
      ...
      {
      case BAND_LIMITED:
        // we are performing BLi in both the x and z directions
        ...
        for (int kk = up; kk <= low; kk++)
        {
          Hx_interps_bli[kk - low] = BLi(Hxy[kk][j][i - 4] + Hxz[kk][j][i - 4],
                                     ...
                                     Hxy[kk][j][i + 1] + Hxz[kk][j][i + 1],
                                     Hxy[kk][j][i + 2] + Hxz[kk][j][i + 2],
                                     Hxy[kk][j][i + 3] + Hxz[kk][j][i + 3]); // BLi to get Hx[i-1/2][j][kk];
        }
        break;
      case INTERP1:
        // we are doing midpoint-cubic interpolation for x, but BLi in the z-direction
        ...
        break;
      case INTERP2:
        // we are doing left-cubic interpolation for x, but BLi in the z-direction
        ...
        break;
      case INTERP3:
        // we are doing right-cubic interpolation for x, but BLi in the z-direction
    ...
    case INTERP1:
      ...
      {
      case BAND_LIMITED:
        // we are doing BLI in the x-direction, and interp1 in the z-direction
        for (int kk = up; kk <= low; kk++)
        {
        ...
        break;
      case INTERP1:
        // we are doing interp1 in both the x and z directions
        for (int kk = low; kk <= up; kk++)
        {
        ...
        break;
      case INTERP2:
        // we are doing interp2 in x and interp1 in z
        for (int kk = low; kk <= up; kk++)
        {
        ...
        break;
      case INTERP3:
        // we are doing interp3 in x and interp1 in z
        for (int kk = low; kk <= up; kk++)
        {
    ...
    case INTERP2:
      ...
      {
      case BAND_LIMITED:
        // we are doing BLI in the x-direction, and interp2 in the z-direction
        for (int kk = up; kk <= low; kk++)
        {
          Hx_interps[kk - low] = BLi(Hxy[kk][j][i - 4] + Hxz[kk][j][i - 4],
                                     Hxy[kk][j][i - 3] + Hxz[kk][j][i - 3],
        ...
        break;
      case INTERP1:
        // we are doing interp1 in x and interp2 in z
        for (int kk = low; kk <= up; kk++)
        {
          Hx_interps[kk - low] = interp1(Hxy[kk][j][i - 2] + Hxz[kk][j][i - 2],
                                         Hxy[kk][j][i - 1] + Hxz[kk][j][i - 1],
      ...
      case INTERP2:
        ...
        break;
      case INTERP3:
        // we are doing interp3 in x and interp2 in z
        for (int kk = low; kk <= up; kk++)
        {
          Hx_interps[kk - low] = interp3(Hxy[kk][j][i - 3] + Hxz[kk][j][i - 3],
                                         Hxy[kk][j][i - 2] + Hxz[kk][j][i - 2],
    ...
    case INTERP3:
      ...
      case BAND_LIMITED:
        ...
        break;
      case INTERP1:
        // we are doing interp1 in x and interp3 in z
        for (int kk = low; kk <= up; kk++)
        {
          Hx_interps[kk - low] = interp1(Hxy[kk][j][i - 2] + Hxz[kk][j][i - 2],
                                         Hxy[kk][j][i - 1] + Hxz[kk][j][i - 1],
        ...
        break;
      case INTERP2:
        // we are doing interp2 in x and interp3 in z
        for (int kk = low; kk <= up; kk++)
        {
          Hx_interps[kk - low] = interp2(Hxy[kk][j][i - 1] + Hxz[kk][j][i - 1],
                                         Hxy[kk][j][i] + Hxz[kk][j][i],
