...
TEST_CASE("Test file I/O construction/destruction.") {
  ...
  SECTION("Check file I/O modes") {
    // check possible file i/o modes: create file, then open same file readonly,
    // then open rw, then create it again (even though it already exists)
    for (auto i : {HDF5FileMode::OVERWRITE, HDF5FileMode::READONLY,
                   HDF5FileMode::READWRITE, HDF5FileMode::OVERWRITE}) {
      SPDLOG_DEBUG("Constructing in mode {}", i);
      HDF5File fi(tmp.string() + "/test_file_constructor.h5",
                  static_cast<HDF5FileMode>(i));
      CHECK(fi.isOK());
  ...
  SECTION("Check can't open nonexistent file") {
    CHECK_THROWS(HDF5File(tmp.string() + "/this_file_doesnt_exist.h5",
                          HDF5FileMode::READONLY));
  }
  ...
  SECTION("Check can't open file readonly and write to it") {
    // create a file
    {
      HDF5File f1(tmp.string() + "/test_file_read_only.h5",
                  HDF5FileMode::OVERWRITE);
    }// destructor called as we leave scope

    HDF5File f2(tmp.string() + "/test_file_read_only.h5",
                HDF5FileMode::READONLY);
