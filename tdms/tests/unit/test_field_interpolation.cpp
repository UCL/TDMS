...
TEST_CASE("E-field interpolation check") {
    ...
    double ***Ex_exact, ***Ex_interp, ***Ey_exact, ***Ey_interp, ***Ez_exact, ***Ez_interp;
    // allocate appropriate memory here?
    ...
    for(int ii=0; ii<Nx; ii++) {
        for(int jj=0; jj<Ny; jj++) {
            for(int kk=0; kk<Nz; kk++) {
                ...
                Exy[kk][jj][ii] = 0.5 * c2pi(x[2 * ii + 1]) * s2pi(y[2 * jj]) * s2pi(z[2 * kk]);
                Exz[kk][jj][ii] = 0.5 * c2pi(x[2 * ii + 1]) * s2pi(y[2 * jj]) * s2pi(z[2 * kk]);
                // y components EY_{ii,jj,kk} := Ex[2*ii, 2*jj+1, 2*kk]
                Eyx[kk][jj][ii] = 0.5 * s2pi(x[2 * ii]) * c2pi(y[2 * jj + 1]) * s2pi(z[2 * kk]);
                Eyz[kk][jj][ii] = 0.5 * s2pi(x[2 * ii]) * c2pi(y[2 * jj + 1]) * s2pi(z[2 * kk]);
                // z components EZ_{ii,jj,kk} := Ex[2*ii, 2*jj, 2*kk+1]
                Ezx[kk][jj][ii] = 0.5 * s2pi(x[2 * ii]) * s2pi(y[2 * jj]) * c2pi(z[2 * kk + 1]);
                Ezy[kk][jj][ii] = 0.5 * s2pi(x[2 * ii]) * s2pi(y[2 * jj]) * c2pi(z[2 * kk + 1]);
    ...
    for(int ii=1; ii<Nx; ii++) {
        for(int jj=1; jj<Ny; jj++) {
            for(int kk=1; kk<Nz; kk++) {
                ...
                interpolateTimeDomainEField(Exy, Exz, Eyx, Eyz, Ezx, Ezy, ii, jj, kk, Nx, Ny, Nz, Ex_val, Ey_val, Ez_val);
                // save interpolated values to the interp array
                Ex_interp[kk][jj][ii] = *Ex_val;
                Ey_interp[kk][jj][ii] = *Ey_val;
                Ez_interp[kk][jj][ii] = *Ez_val;
            }
        }
    }
    ...
    std::cout << "ii, jj, kk \t | x-diff \t | y-diff \t | z-diff \n";
    for (int ii = 1; ii < Nx; ii++)
    {
        for (int jj = 1; jj < Ny; jj++)
        {
            for (int kk = 1; kk < Nz; kk++)
            {
                ...
                std::cout << jj + ", ";
                std::cout << kk + " \t | ";
                std::cout << abs(Ex_exact[kk][jj][ii] - Ex_interp[kk][jj][ii]) + "\t | ";
                std::cout << abs(Ey_exact[kk][jj][ii] - Ey_interp[kk][jj][ii]) + "\t | ";
                std::cout << abs(Ez_exact[kk][jj][ii] - Ez_interp[kk][jj][ii]) + "\n";
            }
        }
    }
}