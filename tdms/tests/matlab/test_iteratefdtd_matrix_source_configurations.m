...
function testIteratefdtdMatrixSourceCombinations(testcase)
    ...
    % The 2*2*2*2*2 array case_errors is indexed as (TD-field, usecd, compactsource, efname, hfname), using index 1 -> that variable takes the value logical 0 and 2->1 in similar fashion. Entries are also boolean flags: zeros error should be raised for the particular combination of (TD-field, usecd, compactsource, efname, hfname) that makes up the index. 1s obviously indicate the converse - an IncompatibleSourceInput error should be raised.
