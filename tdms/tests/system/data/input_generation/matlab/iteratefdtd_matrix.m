...
'intmatprops', 1, ...
'use_bli', 0, ...
'intmethod', 1, ... % Remove in CLI updates
'tdfdir', '', ...
'fieldsample', struct('i',[],'j',[],'k',[],'n',[]), ...
'campssample', struct('vertices',[],'components',[]), ...
'use_pstd', 0, ...
'usecd', 1, ... %Remove in CLI updates
'compactsource', 1 ...
);
[z_launch, runmode, dimension, phasorinc, ...
multilayer, kappa_max, vc_vec, wp_vec, ...
structure, f_ex_vec, intphasorssurface, exdetintegral, ...
air_interface, intmatprops, use_bli, intmethod, tdfdir, ...
fieldsample, campssample, use_pstd, usecd, compactsource] = get_from_input_file(input_file, optional_args, ...
'z_launch', 'runmode', 'dimension', 'phasorinc', ...
'multilayer', 'kappa_max', 'vc_vec', 'wp_vec', ...
'structure', 'f_ex_vec', 'intphasorssurface', 'exdetintegral', ...
'air_interface', 'intmatprops', 'use_bli', 'intmethod', 'tdfdir', ...
'fieldsample', 'campssample', 'use_pstd', 'usecd', 'compactsource');
...
using_fdtd = (usecd==1);%% THIS NEEDS TO USE THE NEWER FLAGS IN AN UPDATE! usecd =1 when using FDTD
...
if strcmp(operation, 'illsetup')
	...
	if strcmp(operation,'filesetup')
		...
		'D_tilde','k_det_obs_global','air_interface','intmatprops','use_bli','intmethod',...
		'tdfield','tdfdir','fieldsample','campssample','use_pstd','usecd',...
		'-v7.3');
	else
