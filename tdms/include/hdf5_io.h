 ...
 */
enum HDF5FileMode { READONLY = 0, READWRITE, OVERWRITE };
...
public:
  ...
  /** Normal constructor. */
  HDF5File(const std::string &filename, HDF5FileMode mode)
...
private:
  /** Common to both constructors: open/create the file and set file_ to point
   * to it. */
  void _open();
  ...
  std::vector<H5::DataSet *>
          datasets_; /**< All datasets to be written to the file. */
};
