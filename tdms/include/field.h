...
/**
 * @brief Electric-split-field components.
 *
 * Interpolation methods are different for electric and magnetic fields, so they
 * require separate interpolation methods.
 */
class ElectricSplitField : public SplitField {
protected:
  //! Electric split field DFT involves one fewer point than its magnetic
  //! counterpart
  int delta_n() override { return -1; };
...
/**
 * @brief Magnetic-split-field components.
 *
 * Interpolation methods are different for electric and magnetic fields, so they
 * require separate interpolation methods.
 */
class MagneticSplitField : public SplitField {
protected:
  //! Magnetic split field DFT involves one greater point than its electric
  //! counterpart
  int delta_n() override { return 0; };
...
/**
 * @brief Current-density split-field components.
 *
 * Whilst inheriting the SplitField structure, this class does not utilise the
 * interpolation functionality
 */
class CurrentDensitySplitField : public SplitField {
protected:
  //! Need to provide a definition for this virtual function
  int delta_n() override { return 0; }
...
public:
  ...
  //! Need to provide a definition for this virtual function, even though it is
  //! not utilised
  double interpolate_to_centre_of(AxialDirection d,
                                  CellCoordinate cell) override {
    return 0.;
...
public:
  ...
  /*! Upper (u) and lower (l) indices in the x,y,z directions.
  EG: il is the first non-pml cell in the i direction and iu the last in the
  corresponding split field grid */
  int il = 0, iu = 0, jl = 0, ju = 0, kl = 0, ku = 0;
  ...
  /**
   * @brief Construct a new Field object with the given dimensions
   *
   * @param I_total,J_total,K_total Dimensions to initialise with
   */
  Field(int I_total, int J_total, int K_total);

  /**
   * @brief Allocate the memory appropriate for all the 3D tensors associated
   * with this split field
   */
  void allocate();

  /**
   * @brief Set all the values of all components of the field to zero
   */
  void zero();

  /**
   * @brief Allocate and set to zero all components of the field
   */
  void allocate_and_zero() {
    allocate();
    zero();
