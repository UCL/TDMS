...
The test data contains "reference" input files which in turn provide arrays for: the incident electric field, the computational grid, etc. which are needed by the simulation, and have been generated by a trusted version of the relevant MATLAB scripts.
The data also contains "reference" output files which have been generated by a trusted version of TDMS - the system tests check that any development changes have preserved the functionality of TDMS.

<details>
<summary> **Planned changes regarding MATLAB removal** </summary>
\note [`test_regen.py`](https://github.com/UCL/TDMS/blob/main/tdms/tests/system/test_regen.py) will replace `test_system.py` and `read_config.py` when the [input overhaul](https://github.com/UCL/TDMS/issues/70) is complete. Due to difficulties running `MATLAB` from within `Python` on CI (see [#239](https://github.com/UCL/TDMS/issues/239)), we can only run `test_system.py` on CI, which is why the test data contains input `.mat` files as well as the corresponding outputs.

If you are developing, you can run `test_regen.py` yourself in place of `test_system.py` by invoking `pytest`. However you will need to [install `MatlabEngine`](https://uk.mathworks.com/help/matlab/matlab_external/install-the-matlab-engine-for-python.html) into the environment you installed the `requirements.txt` first.
</details>
