...
You'll then need to [compile](#compiling) `tdms`, and once that is done the system tests can be run by invoking `pytest` and pointing it to the `tdms/tests/system` directory:
```{.sh}
$ pwd
/path/to/repository/TDMS
...
When you run the tests for the first time, the example data will be downloaded to `tdms/tests/system/data` (and will be [ignored by git](https://github.com/UCL/TDMS/blob/main/.gitignore)).
Subsequent runs of the test will not re-download unless you manually delete the zip file(s).
