...
x = linspace(0,2,N+1);xhr = linspace(0,2,500);xhr = xhr(1:(end-1));
x = x(1:(end-1));
dx = diff(x(1:2));
...
W = .25;
xg = 1;
mask = double(x>-1);maskhr = double(xhr>-1);
for i=1:Ns
...
%G3_d_a = FFT_derivative_3d(G3_a,dx,delta*dx,3);
G3_d_a = FFT_derivative_3d(G3_a,1,delta,3);
G3_d_a = G3_d_a/dx;
g_d_n =  numerical_derivative(g_a,delta)/N/dx;
